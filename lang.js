...
// ========== BSA ==========
'bsa.title': { ko: '📋 6개월 계약 입찰 안건!', ja: '📋 6ヶ月契約 入札案件！' },
...
'bsa.bid0': { ko: '💪 정가 입찰 (승률 {0}%)', ja: '💪 定価入札（勝率 {0}%）' },
'bsa.bid10': { ko: '📉 10% 할인 입찰 (승률 {0}%)', ja: '📉 10%割引入札（勝率 {0}%）' },
'bsa.bid20': { ko: '📉 20% 할인 입찰 (승률 {0}%)', ja: '📉 20%割引入札（勝率 {0}%）' },
'bsa.bid30': { ko: '📉 30% 할인 입찰 (승률 {0}%)', ja: '📉 30%割引入札（勝率 {0}%）' },
'bsa.decline': { ko: '❌ 입찰 포기', ja: '❌ 入札放棄' },
'bsa.resultTitle': { ko: '📋 입찰 결과', ja: '📋 入札結果' },
'bsa.resultWon': { ko: '🏆 낙찰 성공!', ja: '🏆 落札成功！' },
'bsa.resultLost': { ko: '❌ 입찰 실패', ja: '❌ 入札失敗' },
'bsa.myBid': { ko: '내 입찰가', ja: '自社入札額' },
'bsa.perVoy': { ko: '/항차', ja: '/航次' },
'bsa.competitor': { ko: '경쟁사', ja: '競合他社' },
'bsa.winner': { ko: '낙찰', ja: '落札' },
'bsa.loser': { ko: '탈락', ja: '脱落' },
'bsa.resultConfirm': { ko: '확인', ja: '確認' },
'bsa.contractStart': { ko: '계약 기간: {0}개월 ({1}항차)', ja: '契約期間: {0}ヶ月（{1}航次）' },
'bsa.expectedRevenue': { ko: '총 예상 수익: ${0}', ja: '総予想収益: ${0}' },
'bsa.flowForward': { ko: '기기 순방향', ja: '機器順方向' },
'bsa.flowReverse': { ko: '기기 역방향', ja: '機器逆方向' },
'bsa.won': { ko: '🏆 낙찰! 항차당 {0}TEU × {1}항차 계약 체결!', ja: '🏆 落札！航次あたり {0}TEU × {1}航次 契約締結！' },
'bsa.wonToast': { ko: '계약 체결! ${0}', ja: '契約締結！${0}' },
'bsa.lost': { ko: '😞 입찰 실패... 경쟁사에 낙찰되었습니다.', ja: '😞 入札失敗... 競合他社に落札されました。' },
'bsa.lostToast': { ko: '입찰 실패', ja: '入札失敗' },
...
'ctr.homeExcess': { ko: '⚠ {0} 공컨테이너 과다 ({1}개, 적정 {2}개) — 영업 확대 또는 타 항구 배분 필요', ja: '⚠ {0} 空コンテナ過多（{1}個、適正{2}個）— 営業拡大または他港への配分が必要' },
...
'ticker.typhoonWarn': { ko: '🌀 태풍 접근 중! 연료비 +30% 발생 가능 — 출항 전 연료비 확인', ja: '🌀 台風接近中！燃料費+30%発生可能 — 出港前に燃料費確認' },
'ticker.highWave': { ko: '🌊 {0} 높은 파도 경보 — 연료비 +15% 발생 가능', ja: '🌊 {0} 高波警報 — 燃料費+15%発生可能' },
'ticker.marketBoom': { ko: '🔥 {0} 시장 성수기! 운임·물량 급등 중 — 영업 집중 추천', ja: '🔥 {0} 市場繁忙期！運賃・物量急騰中 — 営業集中推奨' },
'ticker.marketSlump': { ko: '📉 {0} 비수기 — 운임·물량 하락 중. 다른 항구에 영업력 집중 검토', ja: '📉 {0} 閑散期 — 運賃・物量下落中。他港への営業集中を検討' },
'ticker.oilSpike': { ko: '🛢️ 유가 급등! $\{0\}/배럴 — 연료비 부담 증가 주의', ja: '🛢️ 原油急騰！$\{0\}/バレル — 燃料費負担増加に注意' },
'ticker.oilCheap': { ko: '🛢️ 유가 하락 $\{0\}/배럴 — 연료비 절감 기회!', ja: '🛢️ 原油下落 $\{0\}/バレル — 燃料費節減のチャンス！' },
'oil.barrel': { ko: '유가', ja: '原油価格' },
'oil.high': { ko: '고유가', ja: '原油高' },
'oil.fuelExtra': { ko: '연료비 할증', ja: '燃料費割増' },
'safety.riskDown': { ko: '🛡️ 위험 운항 선택 — 안전 점수 하락 ({0}점)', ja: '🛡️ 危険運航選択 — 安全スコア低下（{0}点）' },
'safety.safeUp': { ko: '🛡️ 안전 운항 선택 — 안전 점수 상승 ({0}점)', ja: '🛡️ 安全運航選択 — 安全スコア上昇（{0}点）' },
...
'theme.upgrade': { ko: '회사 등급 승격!', ja: '会社ランク昇格！' },
};
